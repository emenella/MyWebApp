            ...
            {data.socials?.data.map((link, index) => (
                    <a href={link.attributes?.url} key={index} className={badgeVariants({variant: "default"}) + ""} target="_blank" rel="noopener noreferrer">
                        <div className="flex flex-row items-center gap-2">
                            {isClientSide ? <Image src={getUrlImage(link.attributes?.Icon?.Media?.data?.attributes?.url || "")} alt={`Icon for ${link.attributes?.name}`} width={30} height={30} className={invert(link.attributes?.Icon?.Invert || false, resolvedTheme, "")}/> : <Skeleton className="h-6 w-6 rounded-full"/>}
